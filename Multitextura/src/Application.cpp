...
void Application::initTexture()
{
	...
	/*if (text == 1)
	unsigned char* img = SOIL_load_image("Mya.jpg", &img_width, &img_height, &channels, 0);
	*/
...
void Application::initTexture2()
{
	...
	unsigned char* img = SOIL_load_image("Mya.jpg", &img_width, &img_height, &channels, 0);  //cambia a skeletorcitos
...
void Application::setup()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, PlanoPlox.vbo);              //hacer 2 buffer?
	//buffer para lenna
...
void Application::display()
{
	...
	//Seleccionamos los shaders a usar, lenna
	glUseProgram(PlanoPlox.shader[0]);
	//skeletr
	glUseProgram(PlanoPlox.shader[1]);
	glm::mat4 transform = camera;// *PlanoPlox.rotation;
	transform = glm::perspective(45.0f, 640.0f/480.0f, 0.1f,200.0f)*transform*mTra;
	glUniformMatrix4fv(PlanoPlox.uTransform[0],1,GL_FALSE, glm::value_ptr(transform));

	glUniform1f(PlanoPlox.uTime[0] ,fTime);
	glUniform3fv(PlanoPlox.Light[0], 1, glm::value_ptr(PlanoPlox.Light));
	glUniform3fv(PlanoPlox.Ojo[0], 1, glm::value_ptr(PlanoPlox.Ojo));
...
{
	PlanoPlox.aRotations.y = ((Xpos + WIDTH) / 50.0f);
